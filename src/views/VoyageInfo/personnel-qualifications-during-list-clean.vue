<template>
	<div class="personnel-qualifications-during-list">
		<!-- 页面标题和操作按钮 -->
		<div class="page-header">
			<h2>外业调查人员资质一览表(航中)</h2>
		...
		<div class="container">
			...
			>
				...
				<!-- 性别列 -->
				<template #sex="{ row }">
					<el-tag :type="row.sex === '男' ? 'primary' : 'success'">
						{{ row.sex }}
					</el-tag>
				</template>
				...
				<el-row :gutter="20">
					...
					<el-col :span="12">
						<el-form-item label="本航次操作仪器" prop="instruments">
							<el-input v-model="formData.instruments" placeholder="请输入操作仪器" />
						</el-form-item>
					</el-col>
				</el-row>
				<el-form-item label="培训情况" prop="training">
					<el-input v-model="formData.training" type="textarea" :rows="3" placeholder="请输入培训情况" />
				</el-form-item>
		...
		<!-- 查看详情弹窗 -->
		<el-dialog title="人员资质详情" v-model="visible1" width="800px">
...
<script setup lang="ts" name="PersonnelQualificationsDuringList">
import { ref, reactive, computed, defineProps } from 'vue';
import { ElMessage, ElMessageBox, FormInstance } from 'element-plus';
import { CirclePlusFilled, Download, Upload } from '@element-plus/icons-vue';
import {
	fetchPersonnelQualificationsDuring,
	createPersonnelQualificationDuring,
	updatePersonnelQualificationDuring,
	deletePersonnelQualificationDuring,
	batchDeletePersonnelQualificationsDuring,
	downloadExcelTemplate,
	importExcelFile,
	exportPersonnelQualifications
} from '@/api/personnelDuring';
import TableCustom from '@/components/table-custom.vue';
import TableSearch from '@/components/table-search.vue';
import { TableItem } from '@/types/table';

// 航中检查人员资质数据类型
interface PersonnelQualificationDuring {
    id: number;
    task_name: string;
    name: string;
    sex: string;
...
const tableData = ref<PersonnelQualificationDuring[]>([]);
const selectedRows = ref<PersonnelQualificationDuring[]>([]);
...
const getData = async () => {
	try {
		...
		const response = await fetchPersonnelQualificationsDuring(params);
...
// 编辑
const handleEdit = (row: PersonnelQualificationDuring) => {
	isEdit.value = true;
	formData.value = { ...row };
	visible.value = true;
};
...
// 删除
const handleDelete = async (row: PersonnelQualificationDuring) => {
	try {
		...
		await deletePersonnelQualificationDuring(row.id);
...
const handleBatchDelete = async () => {
	...
	try {
		...
		await batchDeletePersonnelQualificationsDuring(ids);
...
const handleSubmit = async () => {
	try {
		...
		if (isEdit.value) {
			await updatePersonnelQualificationDuring(formData.value.id, formData.value);
			ElMessage.success('更新成功');
		} else {
			await createPersonnelQualificationDuring(formData.value);
...
const viewData = ref<PersonnelQualificationDuring | null>(null);

const handleView = (row: PersonnelQualificationDuring) => {
	viewData.value = row;
	visible1.value = true;
};
