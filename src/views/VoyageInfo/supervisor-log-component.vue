<template>
    <div class="supervisor-log">
        ...
        >
            ...
            <el-table-column prop="supervisor" label="监督员" width="120" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.supervisor" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.supervisor }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="inspection_date" label="检查日期" width="120" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                    ...
                    <span v-else>{{ row.inspection_date }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="inspection_content" label="检查内容" min-width="200">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.inspection_content" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.inspection_content }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="existing_problems" label="存在问题" min-width="200">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.existing_problems" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.existing_problems }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="rectification_status" label="整改情况" min-width="200">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.rectification_status" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.rectification_status }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="form_filling_time" label="填表时间" width="150" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-date-picker :disabled="isReadonly"
                            v-model="row.form_filling_time"
                            type="datetime"
                            placeholder="选择时间"
                            size="small"
                    ...
                    <span v-else>{{ row.form_filling_time }}</span>
                </template>
            </el-table-column>
        ...
        >
            <el-form :model="formData" :rules="formRules" ref="formRef" label-width="120px">
                <el-row :gutter="20">
                    ...
                    <el-col :span="12">
                        <el-form-item label="监督员" prop="supervisor">
                            <el-input :disabled="isReadonly" v-model="formData.supervisor" placeholder="请输入监督员" />
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="12">
                        <el-form-item label="填表时间" prop="form_filling_time">
                            <el-date-picker :disabled="isReadonly"
                                v-model="formData.form_filling_time"
                                type="datetime"
                                placeholder="选择填表时间"
                ...
                <el-form-item label="检查内容" prop="inspection_content">
                    <el-input :disabled="isReadonly"
                        v-model="formData.inspection_content"
                        type="textarea"
                        :rows="3"
                        placeholder="请输入检查内容"
                    />
                ...
                <el-form-item label="存在问题" prop="existing_problems">
                    <el-input :disabled="isReadonly"
                        v-model="formData.existing_problems"
                        type="textarea"
                        :rows="3"
                        placeholder="请输入存在问题"
                    />
                ...
                <el-form-item label="整改情况" prop="rectification_status">
                    <el-input :disabled="isReadonly"
                        v-model="formData.rectification_status"
                        type="textarea"
                        :rows="3"
                        placeholder="请输入整改情况"
                    />
...
import {
  fetchSupervisorLogList,
  createSupervisorLog,
  updateSupervisorLog,
  deleteSupervisorLog,
  batchDeleteSupervisorLog,
  uploadSupervisorLogAttachment
} from '@/api/supervisor-log'
...
const formRules = {
  ...
  supervisor: [
    { required: true, message: '请输入监督员', trigger: 'blur' }
  ],
  inspection_date: [
...
const loadData = async () => {
  try {
    ...
    const res = await fetchSupervisorLogList(params)
    if (res.code === 200) {
      // 为每行数据添加编辑状态
...
const handleSave = async (row: any) => {
  try {
    const data = {
      ...
      existing_problems: row.existing_problems,
      rectification_status: row.rectification_status,
      form_filling_time: row.form_filling_time,
      attachmentList: row.attachmentList
    }
    
    let res
    if (row.id) {
      res = await updateSupervisorLog(row.id, data)
    } else {
      res = await createSupervisorLog(data)
    }
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要删除这条监督员日志记录吗？', '提示', {
    ...
    if (row.id) {
      const res = await deleteSupervisorLog(row.id)
      if (res.code === 200) {
...
const handleDeleteBatch = async () => {
  ...
  try {
    ...
    if (ids.length > 0) {
      const res = await batchDeleteSupervisorLog(ids)
      if (res.code === 200) {
...
const handleAttachmentUpload = async (params: any, row: any) => {
  try {
    ...
    const response = await uploadSupervisorLogAttachment(formData)
...
const handleDialogSubmit = async () => {
  try {
    ...
    const data = {
      ...
      inspection_content: formData.inspection_content,
      existing_problems: formData.existing_problems,
      rectification_status: formData.rectification_status,
      form_filling_time: formData.form_filling_time,
      attachmentList: formData.attachmentList
    }
    
    const res = await createSupervisorLog(data)
    if (res.code === 200) {
...
const handleDialogAttachmentUpload = async (file: any) => {
  try {
    ...
    const response = await uploadSupervisorLogAttachment(uploadFormData)
    if (response.code === 200) {
...
const handleExport = () => {
  try {
    const exportData = tableData.value.map(item => ({
      ...
      '监督员': item.supervisor,
      '检查日期': item.inspection_date,
      '检查内容': item.inspection_content,
      '存在问题': item.existing_problems,
      '整改状态': item.rectification_status,
      '填表时间': item.form_filling_time,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, '监督员日志')
    XLSX.writeFile(workbook, `监督员日志_${props.taskName}.xlsx`)
