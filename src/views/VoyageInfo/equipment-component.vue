...
const loadData = async () => {
    try {
        console.log('开始加载仪器设备数据，任务名称:', props.taskName);

        const response = await fetchEquipmentList({ task_name: props.taskName });
...
const handleSave = async (row: Equipment) => {
    try {
        ...
        if (row.id) {
            // 更新现有记录
            const response = await updateEquipment(row.id, updateData);
            if (response && response.code === 200) {
            ...
            // 创建新记录
            const response = await createEquipment(updateData);
            if (response && response.code === 200) {
...
// 取消编辑
const handleCancel = (row: Equipment) => {
    if (row.id) {
        // 如果是编辑现有记录，取消编辑状态
        row.isEditing = false;
    } else {
...
const handleDeleteBatch = async () => {
    ...
    try {
        ...
        if (confirmResult === 'confirm') {
            ...
            if (ids.length > 0) {
                const response = await batchDeleteEquipment(ids);
                if (response && response.code === 200) {
...
// 附件上传处理
const handleAttachmentUpload = async (params: any, row: Equipment) => {
    try {
        const formData = new FormData();
        formData.append('file', params.file);

        const response = await uploadEquipmentAttachment(formData);
        if (response && response.code === 200) {
            // 初始化attachmentList
...
// 查看附件
const handleViewAttachment = (row: Equipment) => {
...
const handleExport = () => {
    try {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, '仪器设备一览表');
        XLSX.writeFile(workbook, `仪器设备一览表_${props.taskName}.xlsx`);
        ...
        if (valid) {
            try {
                ...
                if (isEdit.value && editingId.value) {
                    // 编辑模式
                    const response = await updateEquipment(editingId.value, submitData);
                    if (response && response.code === 200) {
                    ...
                    // 新增模式
                    const response = await createEquipment(submitData);
                    if (response && response.code === 200) {
                        ElMessage.success('创建成功');
                        dialogVisible.value = false;
                        loadData();
