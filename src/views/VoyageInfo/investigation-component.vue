...
interface InvestigationProject {
    ...
    unit_a_instrument: string;
    unit_b_instrument: string;
...
const selectedRows = ref<InvestigationProject[]>([]);
const page = reactive({
    current: 1,
    size: 10,
    total: 0
...
const dialogTitle = ref('新增调查项目');
const isEdit = ref(false);
const editingId = ref<number | null>(null);

// 表单数据
const formData = reactive<InvestigationProject>({
...
const formRules = {
    ...
    investigation_item: [{ required: true, message: '请输入调查项目/仪器', trigger: 'blur' }],
    unit_a_instrument: [{ required: true, message: '请输入比测单位甲仪器', trigger: 'blur' }],
    unit_b_instrument: [{ required: true, message: '请输入比测单位乙仪器', trigger: 'blur' }]
};
...
const loadData = async () => {
    try {
        console.log('开始加载外业调查项目数据，任务名称:', props.taskName);

        const response = await fetchInvestigationList({ task_name: props.taskName });
...
// 表格选择变化
const handleSelectionChange = (selection: InvestigationProject[]) => {
    selectedRows.value = selection;
};

// 行点击事件
const handleRowClick = (row: InvestigationProject) => {
    // 如果点击的是操作列，不触发跳转
    if (row.isEditing) {
        return;
    }
...
// 编辑行
const handleEdit = (row: InvestigationProject) => {
    // 先保存当前正在编辑的行
...
// 保存行
const handleSave = async (row: InvestigationProject) => {
    try {
        const updateData = {
            task_name: row.task_name,
            investigation_item: row.investigation_item,
            unit_a_instrument: row.unit_a_instrument,
            unit_b_instrument: row.unit_b_instrument,
        ...
        if (row.id) {
            // 更新现有记录
            const response = await updateInvestigation(row.id, updateData);
            if (response && response.code === 200) {
            ...
            // 创建新记录
            const response = await createInvestigation(updateData);
            if (response && response.code === 200) {
...
// 取消编辑
const handleCancel = (row: InvestigationProject) => {
    if (row.id) {
        // 如果是编辑现有记录，取消编辑状态
        row.isEditing = false;
    } else {
...
// 删除行
const handleDelete = async (row: InvestigationProject) => {
    try {
        const confirmResult = await ElMessageBox.confirm(
            `确定要删除调查项目"${row.investigation_item}"吗？此操作不可撤销。`,
            '删除确认',
            {
        ...
        if (confirmResult === 'confirm') {
            if (row.id) {
                const response = await deleteInvestigation(row.id);
                if (response && response.code === 200) {
...
const handleDeleteBatch = async () => {
    ...
    try {
        ...
        if (confirmResult === 'confirm') {
            ...
            if (ids.length > 0) {
                const response = await batchDeleteInvestigation(ids);
                if (response && response.code === 200) {
...
// 附件上传处理
const handleAttachmentUpload = async (params: any, row: InvestigationProject) => {
    try {
        const formData = new FormData();
        formData.append('file', params.file);

        const response = await uploadInvestigationAttachment(formData);
        if (response && response.code === 200) {
            // 初始化attachmentList
...
// 查看附件
const handleViewAttachment = (row: InvestigationProject) => {
...
const handleExport = () => {
    try {
        const exportData = tableData.value.map(item => ({
            ...
            '调查项目/仪器': item.investigation_item,
            '比测单位甲仪器': item.unit_a_instrument,
            '比测单位乙仪器': item.unit_b_instrument,
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, '外业调查项目统计表');
        XLSX.writeFile(workbook, `外业调查项目统计表_${props.taskName}.xlsx`);
...
const handleDialogAttachmentUpload = async (params: any) => {
    try {
        ...
        const response = await uploadInvestigationAttachment(uploadFormData);
        if (response && response.code === 200) {
            // 初始化attachmentList
        ...
        if (valid) {
            try {
                const submitData = {
                    ...
                    investigation_item: formData.investigation_item,
                    unit_a_instrument: formData.unit_a_instrument,
                    unit_b_instrument: formData.unit_b_instrument,
                ...
                if (isEdit.value && editingId.value) {
                    // 编辑模式
                    const response = await updateInvestigation(editingId.value, submitData);
                    if (response && response.code === 200) {
                    ...
                    // 新增模式
                    const response = await createInvestigation(submitData);
                    if (response && response.code === 200) {
                        ElMessage.success('创建成功');
                        dialogVisible.value = false;
                        loadData();
