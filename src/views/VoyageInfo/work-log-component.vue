<template>
    <div class="work-log">
        ...
        >
            ...
            <el-table-column prop="survey_project" label="调查项目" width="200" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.survey_project" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.survey_project }}</span>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="work_log" label="工作日志" min-width="200">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.work_log" type="textarea" :rows="2" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.work_log }}</span>
                </template>
            </el-table-column>
        ...
        >
            <el-form :model="formData" :rules="formRules" ref="formRef" label-width="150px">
                <el-row :gutter="20">
                    ...
                    <el-col :span="12">
                        <el-form-item label="调查项目" prop="survey_project">
                            <el-input :disabled="isReadonly" v-model="formData.survey_project" placeholder="请输入调查项目" />
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-form-item label="工作日志" prop="work_log">
                    <el-input :disabled="isReadonly"
                        v-model="formData.work_log"
                        type="textarea"
                        :rows="4"
                        placeholder="请输入工作日志"
                    />
...
const dialogTitle = ref('新增外业调查工作日志抽查表')
...
const loadData = async () => {
  try {
    ...
    const res = await fetchWorkLogList(params)
    if (res.code === 200) {
      // 为每行数据添加编辑状态
...
const handleAddRow = () => {
  dialogTitle.value = '新增外业调查工作日志抽查表'
...
const handleSave = async (row: any) => {
  try {
    const data = {
      ...
      survey_project: row.survey_project,
    ...
    let res
    if (row.id) {
      res = await updateWorkLog(row.id, data)
    } else {
      res = await createWorkLog(data)
    }
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要删除这条外业调查工作日志抽查表记录吗？', '提示', {
...
const handleDeleteBatch = async () => {
  ...
  try {
    ...
    if (ids.length > 0) {
      const res = await batchDeleteWorkLog(ids)
      if (res.code === 200) {
...
const handleAttachmentUpload = async (params: any, row: any) => {
  try {
    ...
    const response = await uploadWorkLogAttachment(formData)
...
const handleDialogSubmit = async () => {
  try {
    ...
    const data = {
      ...
      survey_project: formData.survey_project,
      task_undertaking_unit: formData.task_undertaking_unit,
      work_log: formData.work_log,
    ...
    const res = await createWorkLog(data)
    if (res.code === 200) {
...
const handleDialogAttachmentUpload = async (file: any) => {
  try {
    ...
    const response = await uploadWorkLogAttachment(uploadFormData)
    if (response.code === 200) {
...
const handleExport = () => {
  try {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, '外业调查工作日志抽查表')
    XLSX.writeFile(workbook, `外业调查工作日志抽查表_${props.taskName}.xlsx`)
