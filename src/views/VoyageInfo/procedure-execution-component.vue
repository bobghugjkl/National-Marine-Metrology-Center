<template>
    <div class="procedure-execution">
        ...
        <el-table
            ...
            border
            stripe
            class="procedure-execution-table"
        ...
        >
            ...
            <el-table-column prop="investigation_item_instrument" label="调查项目/仪器" width="200" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
            ...
            <el-table-column prop="task_undertaking_unit" label="任务承担单位" width="180" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.task_undertaking_unit" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.task_undertaking_unit }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="has_operating_procedures" label="是否具有操作规程" width="150" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-select :disabled="isReadonly" v-model="row.has_operating_procedures" size="small" @click.stop>
                            <el-option label="是" value="是"></el-option>
                            <el-option label="否" value="否"></el-option>
                        </el-select>
                    </template>
                    <span v-else>{{ row.has_operating_procedures }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="operating_procedure_name" label="操作规程名称" min-width="200">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.operating_procedure_name" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.operating_procedure_name }}</span>
                </template>
            </el-table-column>
        ...
        >
            <el-form :model="formData" :rules="formRules" ref="formRef" label-width="150px">
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="航次任务名称" prop="task_name">
                            <el-input :disabled="isReadonly" v-model="formData.task_name" placeholder="请输入航次任务名称" />
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="调查项目/仪器" prop="investigation_item_instrument">
                            <el-input :disabled="isReadonly" v-model="formData.investigation_item_instrument" placeholder="请输入调查项目/仪器" />
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="任务承担单位" prop="task_undertaking_unit">
                            <el-input :disabled="isReadonly" v-model="formData.task_undertaking_unit" placeholder="请输入任务承担单位" />
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="是否具有操作规程" prop="has_operating_procedures">
                            <el-select :disabled="isReadonly" v-model="formData.has_operating_procedures" placeholder="请选择" style="width: 100%;">
                                <el-option label="是" value="是"></el-option>
                                <el-option label="否" value="否"></el-option>
                            </el-select>
                        </el-form-item>
                ...
                <el-form-item label="操作规程名称" prop="operating_procedure_name">
                    <el-input :disabled="isReadonly" v-model="formData.operating_procedure_name" placeholder="请输入操作规程名称" />
                </el-form-item>
                    ...
                    <div class="dialog-upload">
                        ...
                        <div v-if="formData.attachmentList && formData.attachmentList.length > 0" class="attachment-list">
                            <div v-for="(attachment, index) in formData.attachmentList" :key="index" class="attachment-item">
                                ...
                                <el-button v-if="!isReadonly" type="danger" size="small" @click="handleDialogAttachmentRemove(index)">
                                    删除
                                </el-button>
                            </div>
                        </div>
...
import {
  fetchProcedureExecutionList,
  createProcedureExecution,
  updateProcedureExecution,
  deleteProcedureExecution,
  batchDeleteProcedureExecution,
  uploadProcedureExecutionAttachment
} from '@/api/procedure-execution'
...
const dialogTitle = ref('新增外业调查操作规程执行统计表')
...
const loadData = async () => {
  try {
    ...
    const res = await fetchProcedureExecutionList(params)
    if (res.code === 200) {
      // 为每行数据添加编辑状态
...
const handleAddRow = () => {
  dialogTitle.value = '新增外业调查操作规程执行统计表'
...
const handleSave = async (row: any) => {
  try {
    const data = {
      ...
      has_operating_procedures: row.has_operating_procedures,
      operating_procedure_name: row.operating_procedure_name,
      remarks: row.remarks,
    ...
    let res
    if (row.id) {
      res = await updateProcedureExecution(row.id, data)
    } else {
      res = await createProcedureExecution(data)
    }
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要删除这条外业调查操作规程执行统计表记录吗？', '提示', {
    ...
    if (row.id) {
      const res = await deleteProcedureExecution(row.id)
      if (res.code === 200) {
...
const handleDeleteBatch = async () => {
  ...
  try {
    ...
    if (ids.length > 0) {
      const res = await batchDeleteProcedureExecution(ids)
      if (res.code === 200) {
...
const handleAttachmentUpload = async (params: any, row: any) => {
  try {
    ...
    const response = await uploadProcedureExecutionAttachment(formData)
...
const handleDialogSubmit = async () => {
  try {
    ...
    const data = {
      ...
      has_operating_procedures: formData.has_operating_procedures,
      operating_procedure_name: formData.operating_procedure_name,
      remarks: formData.remarks,
      attachmentList: formData.attachmentList
    }
    
    const res = await createProcedureExecution(data)
    if (res.code === 200) {
...
const handleDialogAttachmentUpload = async (file: any) => {
  try {
    ...
    const response = await uploadProcedureExecutionAttachment(uploadFormData)
    if (response.code === 200) {
...
const handleExport = () => {
  try {
    const exportData = tableData.value.map(item => ({
      ...
      '是否具有操作规程': item.has_operating_procedures,
      '操作规程名称': item.operating_procedure_name,
      '备注': item.remarks,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, '外业调查操作规程执行统计表')
    XLSX.writeFile(workbook, `外业调查操作规程执行统计表_${props.taskName}.xlsx`)
