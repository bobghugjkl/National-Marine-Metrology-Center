<template>
    <div class="voyage-investigation">
        <!-- 操作栏 -->
        ...
        <el-table
            ...
            border
            stripe
            class="voyage-investigation-table"
        ...
        >
            ...
            <el-table-column prop="investigation_item_instrument" label="调查项目/仪器" min-width="200">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.investigation_item_instrument" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.investigation_item_instrument }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="comparison_unit_a_instrument" label="比测单位甲及仪器" min-width="200">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.comparison_unit_a_instrument" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.comparison_unit_a_instrument }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="comparison_unit_b_instrument" label="比测单位乙及仪器" min-width="200">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.comparison_unit_b_instrument" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.comparison_unit_b_instrument }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="comparison_time" label="比测时间" width="120" align="center">
                <template #default="{ row }">
                    ...
                    <span v-else>{{ row.comparison_time }}</span>
                </template>
            </el-table-column>
        ...
        >
            ...
            >
                <el-row :gutter="20">
                    ...
                    <el-col :span="12">
                        <el-form-item label="调查项目/仪器" prop="investigation_item_instrument">
                            <el-input :disabled="isReadonly" v-model="formData.investigation_item_instrument" placeholder="请输入调查项目/仪器"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="比测单位甲及仪器" prop="comparison_unit_a_instrument">
                            <el-input :disabled="isReadonly" v-model="formData.comparison_unit_a_instrument" placeholder="请输入比测单位甲及仪器"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="比测单位乙及仪器" prop="comparison_unit_b_instrument">
                            <el-input :disabled="isReadonly" v-model="formData.comparison_unit_b_instrument" placeholder="请输入比测单位乙及仪器"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="20">
...
import {
    fetchVoyageInvestigationList,
    createVoyageInvestigation,
    updateVoyageInvestigation,
    deleteVoyageInvestigation,
    batchDeleteVoyageInvestigation,
    uploadVoyageInvestigationAttachment
} from '@/api/voyage-investigation';
import * as XLSX from 'xlsx';

// 接口定义
interface VoyageInvestigationProject {
    id?: number;
    task_name: string;
    investigation_item_instrument: string;
    comparison_unit_a_instrument: string;
    comparison_unit_b_instrument: string;
...
// 响应式数据
const tableData = ref<VoyageInvestigationProject[]>([]);
const selectedRows = ref<VoyageInvestigationProject[]>([]);
const page = reactive({
    current: 1,
    size: 10,
    total: 0
...
const dialogTitle = ref('新增航中调查项目');
const isEdit = ref(false);
const editingId = ref<number | null>(null);

// 表单数据
const formData = reactive<VoyageInvestigationProject>({
    task_name: props.taskName,
    investigation_item_instrument: '',
    comparison_unit_a_instrument: '',
    comparison_unit_b_instrument: '',
    comparison_time: '',
    comparison_location: '',
    comparison_result: '',
    remarks: '',
...
const formRules = {
    ...
    investigation_item_instrument: [{ required: true, message: '请输入调查项目/仪器', trigger: 'blur' }],
    comparison_unit_a_instrument: [{ required: true, message: '请输入比测单位甲及仪器', trigger: 'blur' }],
    comparison_unit_b_instrument: [{ required: true, message: '请输入比测单位乙及仪器', trigger: 'blur' }]
};
...
const loadData = async () => {
    try {
        console.log('开始加载航中外业调查项目数据，任务名称:', props.taskName);

        const response = await fetchVoyageInvestigationList({ task_name: props.taskName });
...
// 表格选择变化
const handleSelectionChange = (selection: VoyageInvestigationProject[]) => {
    selectedRows.value = selection;
};

// 行点击事件
const handleRowClick = (row: VoyageInvestigationProject) => {
    // 如果点击的是操作列，不触发跳转
    if (row.isEditing) {
        return;
    }
...
// 编辑行
const handleEdit = (row: VoyageInvestigationProject) => {
    // 先保存当前正在编辑的行
...
// 保存行
const handleSave = async (row: VoyageInvestigationProject) => {
    try {
        const updateData = {
            task_name: row.task_name,
            investigation_item_instrument: row.investigation_item_instrument,
            comparison_unit_a_instrument: row.comparison_unit_a_instrument,
            comparison_unit_b_instrument: row.comparison_unit_b_instrument,
            comparison_time: row.comparison_time,
            comparison_location: row.comparison_location,
        ...
        if (row.id) {
            // 更新现有记录
            const response = await updateVoyageInvestigation(row.id, updateData);
            if (response && response.code === 200) {
            ...
            // 创建新记录
            const response = await createVoyageInvestigation(updateData);
            if (response && response.code === 200) {
...
// 取消编辑
const handleCancel = (row: VoyageInvestigationProject) => {
    if (row.id) {
        // 如果是编辑现有记录，取消编辑状态
        row.isEditing = false;
    } else {
...
// 删除行
const handleDelete = async (row: VoyageInvestigationProject) => {
    try {
        const confirmResult = await ElMessageBox.confirm(
            `确定要删除航中调查项目"${row.investigation_item_instrument}"吗？此操作不可撤销。`,
            '删除确认',
            {
        ...
        if (confirmResult === 'confirm') {
            if (row.id) {
                const response = await deleteVoyageInvestigation(row.id);
                if (response && response.code === 200) {
...
const handleDeleteBatch = async () => {
    ...
    try {
        ...
        if (confirmResult === 'confirm') {
            ...
            if (ids.length > 0) {
                const response = await batchDeleteVoyageInvestigation(ids);
                if (response && response.code === 200) {
...
// 附件上传处理
const handleAttachmentUpload = async (params: any, row: VoyageInvestigationProject) => {
    try {
        const response = await uploadVoyageInvestigationAttachment(params.file);
        if (response && response.code === 200) {
            // 初始化attachmentList
...
// 查看附件
const handleViewAttachment = (row: VoyageInvestigationProject) => {
...
const handleExport = () => {
    try {
        const exportData = tableData.value.map(item => ({
            ...
            '调查项目/仪器': item.investigation_item_instrument,
            '比测单位甲及仪器': item.comparison_unit_a_instrument,
            '比测单位乙及仪器': item.comparison_unit_b_instrument,
            '比测时间': item.comparison_time,
            '比测地点': item.comparison_location,
            '比测结果': item.comparison_result,
            '备注': item.remarks
        }));

        const worksheet = XLSX.utils.json_to_sheet(exportData);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, '航中外业调查项目统计表');
        XLSX.writeFile(workbook, `航中外业调查项目统计表_${props.taskName}.xlsx`);
        ...
        if (valid) {
            try {
                const submitData = {
                    ...
                    investigation_item_instrument: formData.investigation_item_instrument,
                    comparison_unit_a_instrument: formData.comparison_unit_a_instrument,
                    comparison_unit_b_instrument: formData.comparison_unit_b_instrument,
                    comparison_time: formData.comparison_time,
                    comparison_location: formData.comparison_location,
                    comparison_result: formData.comparison_result,
                ...
                if (isEdit.value && editingId.value) {
                    // 编辑模式
                    const response = await updateVoyageInvestigation(editingId.value, submitData);
                    if (response && response.code === 200) {
                    ...
                    // 新增模式
                    const response = await createVoyageInvestigation(submitData);
                    if (response && response.code === 200) {
                        ElMessage.success('创建成功');
                        dialogVisible.value = false;
                        loadData();
