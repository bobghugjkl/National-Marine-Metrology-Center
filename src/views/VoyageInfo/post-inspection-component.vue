    ...
    <div class="post-inspection">
        ...
        >
            ...
            <el-table-column prop="inspection_content" label="检查内容" min-width="200">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.inspection_content" type="textarea" :rows="2" size="small" @click.stop></el-input>
                    </template>
            ...
            <el-table-column prop="existing_problems" label="存在问题" min-width="200">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.existing_problems" type="textarea" :rows="2" size="small" @click.stop></el-input>
                    </template>
            ...
            <el-table-column prop="rectification_status" label="整改情况" min-width="200">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.rectification_status" type="textarea" :rows="2" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.rectification_status }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="form_filling_time" label="填表时间" width="120" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-date-picker :disabled="isReadonly"
...
import {
  fetchPostInspectionList,
  createPostInspection,
  updatePostInspection,
  deletePostInspection,
  batchDeletePostInspection,
  uploadPostInspectionAttachment
} from '@/api/post-inspection'
...
const loadData = async () => {
  try {
    ...
    const res = await fetchPostInspectionList(params)
    if (res.code === 200) {
      // 为每行数据添加编辑状态
...
const handleSave = async (row: any) => {
  try {
    ...
    let res
    if (row.id) {
      res = await updatePostInspection(row.id, data)
    } else {
      res = await createPostInspection(data)
    }
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要删除这条航后检查表记录吗？', '提示', {
    ...
    if (row.id) {
      const res = await deletePostInspection(row.id)
      if (res.code === 200) {
...
const handleDeleteBatch = async () => {
  ...
  try {
    ...
    if (ids.length > 0) {
      const res = await batchDeletePostInspection(ids)
      if (res.code === 200) {
...
const handleAttachmentUpload = async (params: any, row: any) => {
  try {
    ...
    const response = await uploadPostInspectionAttachment(formData)
...
const handleDialogSubmit = async () => {
  try {
    ...
    const res = await createPostInspection(data)
    if (res.code === 200) {
...
const handleDialogAttachmentUpload = async (file: any) => {
  try {
    ...
    const response = await uploadPostInspectionAttachment(uploadFormData)
    if (response.code === 200) {
...
const handleExport = () => {
  try {
    const exportData = tableData.value.map(item => ({
      ...
      '整改情况': item.rectification_status,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, '航后检查表')
    XLSX.writeFile(workbook, `航后检查表_${props.taskName}.xlsx`)
