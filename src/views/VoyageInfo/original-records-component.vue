<template>
    <div class="original-records">
        ...
        >
            ...
            <el-table-column prop="survey_item" label="调查项目" width="150" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.survey_item" size="small" @click.stop></el-input>
                    </template>
                    <span v-else>{{ row.survey_item }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="task_undertaking_unit" label="任务承担单位" width="150" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
            ...
            <el-table-column prop="station" label="站位" width="120" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.station" size="small" @click.stop></el-input>
                    </template>
            ...
            <el-table-column prop="location" label="地点" width="120" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-input :disabled="isReadonly" v-model="row.location" size="small" @click.stop></el-input>
                    </template>
            ...
            <el-table-column prop="qualified_or_not" label="合格与否" width="100" align="center">
                <template #default="{ row }">
                    <template v-if="row.isEditing">
                        <el-select :disabled="isReadonly" v-model="row.qualified_or_not" size="small" @click.stop>
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="12">
                        <el-form-item label="调查项目" prop="survey_item">
                            <el-input :disabled="isReadonly" v-model="formData.survey_item" placeholder="请输入调查项目" />
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="12">
                        <el-form-item label="站位" prop="station">
                            <el-input :disabled="isReadonly" v-model="formData.station" placeholder="请输入站位" />
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="12">
                        <el-form-item label="地点" prop="location">
                            <el-input :disabled="isReadonly" v-model="formData.location" placeholder="请输入地点" />
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="12">
                        <el-form-item label="合格与否" prop="qualified_or_not">
                            <el-select :disabled="isReadonly" v-model="formData.qualified_or_not" placeholder="请选择" style="width: 100%;">
                                <el-option label="合格" value="合格"></el-option>
                                <el-option label="不合格" value="不合格"></el-option>
                            </el-select>
                        </el-form-item>
...
import {
  fetchOriginalRecordsList,
  createOriginalRecord,
  updateOriginalRecord,
  deleteOriginalRecord,
  batchDeleteOriginalRecord,
  uploadOriginalRecordAttachment
} from '@/api/original-records'
...
const dialogTitle = ref('新增外业调查原始记录抽查表')
...
const loadData = async () => {
  try {
    ...
    const res = await fetchOriginalRecordsList(params)
    if (res.code === 200) {
      // 为每行数据添加编辑状态
...
const handleAddRow = () => {
  dialogTitle.value = '新增外业调查原始记录抽查表'
...
const handleSave = async (row: any) => {
  try {
    ...
    let res
    if (row.id) {
      res = await updateOriginalRecord(row.id, data)
    } else {
      res = await createOriginalRecord(data)
    }
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm('确定要删除这条外业调查原始记录抽查表记录吗？', '提示', {
    ...
    if (row.id) {
      const res = await deleteOriginalRecord(row.id)
      if (res.code === 200) {
...
const handleDeleteBatch = async () => {
  ...
  try {
    ...
    if (ids.length > 0) {
      const res = await batchDeleteOriginalRecord(ids)
      if (res.code === 200) {
...
const handleAttachmentUpload = async (params: any, row: any) => {
  try {
    ...
    const response = await uploadOriginalRecordAttachment(formData)
...
const handleDialogSubmit = async () => {
  try {
    ...
    const res = await createOriginalRecord(data)
    if (res.code === 200) {
...
const handleDialogAttachmentUpload = async (file: any) => {
  try {
    ...
    const response = await uploadOriginalRecordAttachment(uploadFormData)
    if (response.code === 200) {
...
const handleExport = () => {
  try {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, '外业调查原始记录抽查表')
    XLSX.writeFile(workbook, `外业调查原始记录抽查表_${props.taskName}.xlsx`)
